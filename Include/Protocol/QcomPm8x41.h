...
#define QCOM_PM8X41_PROTOCOL_GUID                                              \
  {                                                                            \
    0xb6e811d5, 0x1dce, 0x4ccb,                                                \
    {                                                                          \
      0xaf, 0x21, 0xe9, 0xf7, 0xef, 0x68, 0x60, 0x7b                           \
    }                                                                          \
  }

typedef struct _QCOM_PM8X41_PROTOCOL QCOM_PM8X41_PROTOCOL;

typedef void(EFIAPI *pm8x41_lpg_write_t)(
    uint8_t chan, uint8_t off, uint8_t val);
typedef void(EFIAPI *pm8x41_lpg_write_sid_t)(
    uint8_t sid, uint8_t chan, uint8_t off, uint8_t val);
typedef int(EFIAPI *pm8x41_gpio_get_t)(uint8_t gpio, uint8_t *status);
typedef int(EFIAPI *pm8x41_gpio_get_sid_t)(
    uint8_t sid, uint8_t gpio, uint8_t *status);
typedef int(EFIAPI *pm8x41_gpio_set_t)(uint8_t gpio, uint8_t value);
typedef int(EFIAPI *pm8x41_gpio_set_sid_t)(
    uint8_t sid, uint8_t gpio, uint8_t value);
typedef int(EFIAPI *pm8x41_gpio_config_t)(
    uint8_t gpio, struct pm8x41_gpio *config);
typedef int(EFIAPI *pm8x41_gpio_config_sid_t)(
...
typedef int(EFIAPI *pm8x41_ldo_set_voltage_t)(
    struct pm8x41_ldo *ldo, uint32_t voltage);
typedef int(EFIAPI *pm8x41_ldo_control_t)(
    struct pm8x41_ldo *ldo, uint8_t enable);
...
typedef void(EFIAPI *pm8x41_enable_mpp_t)(
    struct pm8x41_mpp *mpp, enum mpp_en_ctl enable);
typedef void(EFIAPI *pm8x41_enable_mvs_t)(
    struct pm8x41_mvs *mvs, enum mvs_en_ctl enable);
...
typedef uint32_t(EFIAPI *pm8x41_get_voltage_based_soc_t)(
    uint32_t cutoff_vol, uint32_t vdd_max);
...
typedef int(EFIAPI *pm_pwm_config_t)(
    unsigned int duty_us, unsigned int period_us);
...
struct _QCOM_PM8X41_PROTOCOL {
  ...
  pm8x41_div_clock_ctrl_t        pm8x41_div_clock_ctrl;
